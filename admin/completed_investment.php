...
<h6 class="element-header">Completed Investments</h6>
...
<tbody>
 <?php 
$query3 = mysqli_query($db, "SELECT users.fname, users.email, investment.plan, investment.amount, acct_details.currently_invested, investment.status, investment.date FROM users, investment, acct_details WHERE investment.user_id = users.id=acct_details.user_id AND investment.status = 'completed' ORDER BY investment.id DESC");
...
<tr class="odd">
	...
	<td><span class='badge badge-info'> <?php echo $details3['status']; ?></span></td>
