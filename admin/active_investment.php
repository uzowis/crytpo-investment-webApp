...
if(isset($_GET['terminate'])){
	...
	$id = $_GET['id'];
	$user_id = $_GET['terminate'];
	$amount = $_GET['amount'];
	//mysqli_query($db, "UPDATE deposit SET status='confirmed' WHERE id = '$id'");
	mysqli_query($db, "UPDATE acct_details SET available_bal=available_bal+'$amount', currently_invested = currently_invested -'$amount' WHERE user_id='$user_id' ");
	mysqli_query($db, "DELETE FROM investment WHERE id='$id' ");
	echo "<script type='text/javascript'>location.href='active_investment.php';</script>";
}
?>
...
<h6 class="element-header">Active Investments</h6>
...
<th class="sorting" tabindex="0" aria-controls="dataTable1" rowspan="1" colspan="1" aria-label="Plan: activate to sort column ascending" style="width: 270px;">Plan</th>
<th class="sorting" tabindex="0" aria-controls="dataTable1" rowspan="1" colspan="1" aria-label="Amount Invested: activate to sort column ascending" style="width: 270px;">Amount Invested($)</th>
...
<th class="sorting" tabindex="0" aria-controls="dataTable1" rowspan="1" colspan="1" aria-label="Date: activate to sort column descending" style="width: 270px;">Date of Investment</th>
<th class="sorting" tabindex="0" aria-controls="dataTable1" rowspan="1" colspan="1" aria-label="Terminate: activate to sort column ascending" style="width: 270px;">Action</th>
...
<tbody>
 <?php 
$query3 = mysqli_query($db, "SELECT users.fname, users.email, investment.plan, investment.user_id, investment.id, investment.amount, acct_details.currently_invested, investment.status, investment.date FROM users, investment, acct_details WHERE investment.user_id = users.id=acct_details.user_id AND investment.status = 'Active' ORDER BY investment.id DESC");
...
<tr class="odd">
	...
	<td> <?php echo $details3['plan']; ?></td>
	<td> <?php echo '$'.$details3['amount']; ?></td>	
	<td><span class='badge badge-success'> <?php echo $details3['status']; ?></span></td>	
	<td><?php echo $details3['date']; ?></td>
	<td><a  type="submit" class="btn btn-danger btn-rounded"  href="active_investment.php?terminate=<?php echo $details3['user_id']; ?> && amount=<?php echo $details3['amount']; ?> && id=<?php echo $details3['id'];?>">Terminate</a></td>
