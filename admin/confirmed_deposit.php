...
<h6 class="element-header">Confirmed Deposits</h6>
...
<tbody>
 <?php 
$query3 = mysqli_query($db, "SELECT users.fname, users.email, users.bit_wallet, deposit.amount, deposit.user_id, deposit.status, deposit.date FROM users, deposit WHERE deposit.user_id = users.id AND deposit.status = 'confirmed' ORDER BY deposit.id DESC");
$result3 = mysqli_num_rows($query3);
while( $details3 = mysqli_fetch_assoc($query3)){
	$amount = $details3['amount'];
	$user_id = $details3['user_id'];
//	mysqli_query($db, "UPDATE acct_details, deposit SET available_bal=available_bal+'$amount' WHERE acct_details.user_id=$user_id AND deposit.status='confirmed' ");
