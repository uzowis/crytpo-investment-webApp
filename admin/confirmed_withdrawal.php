...
<h6 class="element-header">Confirmed Withdrawals</h6>
...
<tbody>
 <?php 
$query3 = mysqli_query($db, "SELECT users.fname, users.email, users.bit_wallet, withdrawal.amount, withdrawal.status, withdrawal.date, acct_details.available_bal FROM users, withdrawal, acct_details WHERE withdrawal.user_id = users.id=acct_details.user_id AND withdrawal.status = 'confirmed' ORDER BY withdrawal.id DESC");
